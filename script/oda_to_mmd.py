...
Usage:
    python script/oda_to_mmd.py [-h] -o OUTPUT_DIRECTORY -l LOG_DIRECTORY -d ODA_DEFAULT_YML  \
        -t ODA_TEMPLATE_XML [-f FROST_URL] [--mmd-validation [MMD_VALIDATION]] [--xsd-mmd XSD_MMD]
Example:
    python script/oda_to_mmd.py -o '~/Data/Output/' -l '~/Logs/' -d 'oda_default.yml' -t \
...
if __name__ == '__main__':
    ...
    log_info = logging.handlers.RotatingFileHandler(
        logDir / 'oda.log', maxBytes=1000000, backupCount=2
    )
    log_info.setLevel(logging.INFO)
    log_debug = logging.handlers.RotatingFileHandler(
        logDir / 'oda_debug.log', maxBytes=1000000, backupCount=10
    )
    ...
    for station in stations_list:
        if all(k in station for k in ("name", "id")):
            ...
            odajson_to_mmd.process_station(
                station['id'][2:],
                station['name'],
                os.path.join(args.output_dir, station['id']),
                args.oda_default,
                args.oda_mmd_template,
                args.frost_url,
                args.mmd_validation,
                args.xsd_mmd
            )
