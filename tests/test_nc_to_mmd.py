...
from py_mmd_tools.nc_to_mmd import (
    Nc_to_mmd,
    get_short_and_long_names,
    nc_wrapper,
    normalize_iso8601,
    normalize_iso8601_0,
    valid_url,
)
from py_mmd_tools.yaml_to_adoc import (
    nc_attrs_from_yaml,
    repetition_allowed,
    required,
    set_attribute,
...
def test_parent_keyword_arg(dataDir):
    ...
    with pytest.raises(ValueError) as ve:
        req, msg = md.to_mmd(parent="no.met:654e8acf-77b1-4f53-b6bf-0cd6cf94e646")
    assert str(ve.value) == "naming_authority ACDD attribute no.kvet is not valid"
    assert "parent reference already in place" in md.missing_attributes["warnings"][1]

    # pre-existing parent but different
    md = Nc_to_mmd(os.path.join(dataDir, "reference_nc_withparent.nc"), check_only=True)
    req, msg = md.to_mmd(parent="no.met:bce66800-1722-495e-975b-6033abb4da7d")
    assert "not updating" in md.missing_attributes["warnings"][2]
...
def test_get_landing_page_url(dataDir):
    md = Nc_to_mmd(os.path.join(dataDir, "reference_nc.nc"), check_only=True)
    assert (
        md.get_dataset_landing_page_url() == "https://data.met.no/dataset/"
        "b7cb7934-77ca-4439-812e-f560df3fe7eb"
    )
    md = Nc_to_mmd(
        os.path.join(dataDir, "reference_nc_missing_keywords_vocab.nc"), check_only=True
    )
...
def testNc_to_mmd_get_geographic_extent_polygon(dataDir):
    ...
    ncin.geospatial_bounds = (
        "POLYGON ((59.01 1.23, 59.06 1.66, 59.10 2.09, 59.15 2.53, "
    ...
    data = md.get_geographic_extent_polygon(
...
def test_get_related_dataset(dataDir):
    ...
    data = md.get_related_dataset(mmd_yaml["related_dataset"], ncin)
    assert (
        'The global attribute "related_dataset" is malformed'
    ...
    data = md.get_related_dataset(mmd_yaml["related_dataset"], ncin)
    assert (
        "The dataset relation type must be either" in md.missing_attributes["errors"][0]
    )
    ncin.close()
    ...
    data = md.get_related_dataset(mmd_yaml["related_dataset"], ncin)
    assert (
        "missing naming_authority in the identifier"
        in md.missing_attributes["errors"][0]
    )
    ncin.close()
...
def test_checksum(monkeypatch):
    ...
    with monkeypatch.context() as mp:
        mp.setattr(
            "py_mmd_tools.nc_to_mmd.Dataset",
...
def test_create_mmd_1(monkeypatch):
    ...
    assert (
        xml_doc.getroot()
        .find("{http://www.met.no/schema/mmd}alternate_identifier[@type='dummy_type']")
        .text
        == "dummy_id_no1"
    )
    assert (
        xml_doc.getroot()
        .find("{http://www.met.no/schema/mmd}alternate_identifier[@type='other_type']")
        .text
        == "dummy_id_no2"
    )
    # platform
...
def test_get_data_access_dict_with_custom_wms(monkeypatch):
    ...
    with monkeypatch.context() as mp:
        ...
        data = md.get_data_access_dict(
            ncin, add_wms_data_access=True, wms_link="http://test-link"
        )
    assert data[1]["type"] == "OGC WMS"
    assert "http://test-link" in str(data[1]["resource"])
    assert data[1]["wms_layers"] == ["M01", "M01_copy"]
...
def test_get_operational_status(dataDir, monkeypatch):
    ...
    value = md.get_operational_status(mmd_element, ncin)
    assert (
        "The ACDD attribute 'processing_level' in MMD attribute 'operational_status'"
        in md.missing_attributes["errors"][0]
    )
...
def test_nc_wrapper_variable_attrs(dataDir):
    ...
    for var, var_attrs in test_ncin.variables.items():
        for attr in var_attrs.ncattrs():
            assert handle_type_comparison(
                var_attrs.getncattr(attr),
    ...
    for var, var_attrs in test_ncin.variables.items():
        for attr in var_attrs.ncattrs():
            assert handle_type_comparison(
                var_attrs.getncattr(attr), test_json_header.variables[var][attr]
            ), (
                f"Divergence in variable attribute between json and nc header at {var}:{attr},"
                f" nc: {var_attrs.getncattr(attr)} of type {type(var_attrs.getncattr(attr))},"
...
def test_json(dataDir, monkeypatch):
    ...
    tmp = Nc_to_mmd(
        test_json_header, json_input=True, check_only=True, checksum_calculation=True
    )
    tmp.to_mmd()
...
def test_nc_wrapper_ncatters(dataDir):
    ...
    for var in test_ncin.variables:
        assert (
            test_ncin.variables[var].ncattrs()
...
class TestNCAttrsFromYaml(unittest.TestCase):
    ...
    def test_set_attribute__wrong_input(self):
        ...
        with self.assertRaises(ValueError):
            set_attribute(
                mmd_field, val, convention, self.attributes, req="not_required"
            )
    ...
    def test_set_attribute__one_conv_field(self):
        ...
        set_attribute(mmd_field, val, convention, self.attributes, req="not_required")
        self.assertEqual(
            self.attributes["acdd_ext"]["not_required"][0],
            {
        ...
        set_attribute(mmd_field, val, convention, self.attributes, req="not_required")
        self.assertEqual(
            self.attributes["acdd"]["not_required"][0],
            {
                "attribute": "processing_level",
                "comment": "Optional",
                "default": "",
                "description": "A textual description of the processing "
    ...
    def test_set_attribute__two_conv_fields(self):
        ...
        self.assertEqual(
            self.attributes["acdd"]["required"][0],
            {
                "attribute": "id",
                "comment": "Required, and should be UUID. No repetition allowed.",
                "default": "",
                "description": "An identifier for the dataset, provided by and unique within "
        ...
        self.assertEqual(
            self.attributes["acdd"]["required"][1],
            {
                "attribute": "naming_authority",
                "comment": "Required. We recommend using reverse-DNS naming. "
                "No repetition allowed.",
                "default": "",
                "description": "The organisation that provides the initial id (see above) for "
    ...
    def test_set_attributes__nested(self):
        ...
        self.assertEqual(
            self.attributes["acdd"]["required"][0],
            {
                ...
                "default": "",
                "description": "A comma-separated list of keywords and/or "
    ...
    def test_repetition_allowed(self):
        ...
        """
        self.assertTrue(
            repetition_allowed(self.mmd_yaml["temporal_extent"]["start_date"])
        )
    ...
    def test_creator_role_acdd_extension(self):
        ...
        set_attributes(mmd_field, val, self.attributes)
        self.assertEqual(
            self.attributes["acdd_ext"]["not_required"][0]["attribute"], "creator_role"
        )
...
class TestNC2MMD(unittest.TestCase):
    ...
    def test_init_raises_error(self):
        ...
        with self.assertRaises(ValueError):
            Nc_to_mmd(
                os.path.abspath("tests/data/reference_nc.nc"),
                output_file=None,
                check_only=False,
            )
    ...
    def test_default_when_no_acdd_or_acdd_ext(self):
        ...
        value = md.get_acdd_metadata(
            mmd_yaml["metadata_status"], ncin, "metadata_status"
        )
    ...
    def test__get_acdd_metadata__dont_accept_alternatives(self):
        ...
        with self.assertRaises(ValueError) as e:
            md.get_acdd_metadata(
                mmd_yaml["metadata_identifier"], ncin, "metadata_identifier"
            )
        self.assertEqual(
            str(e.exception),
    ...
    def test_polygon_is_not_wkt(self):
        ...
        ncin = Dataset(md.netcdf_file, "w", diskless=True)
        ncin.geospatial_bounds = ""
        md.get_geographic_extent_polygon(mmd_yaml["geographic_extent"]["polygon"], ncin)
        self.assertEqual(
            md.missing_attributes["errors"][0],
    ...
    def test_missing_nc_attrs(self):
        ...
        md = Nc_to_mmd(
            os.path.abspath("tests/data/reference_nc_missing_attrs.nc"), check_only=True
        )
        ncin = Dataset(md.netcdf_file)
        value = md.get_acdd_metadata(
            mmd_yaml["geographic_extent"], ncin, "geographic_extent"
        )
    ...
    def test_missing_geographic_extent_but_provided_as_kwarg(self):
        ...
        md = Nc_to_mmd(
            os.path.abspath("tests/data/reference_nc_missing_rectangle.nc"),
            check_only=True,
        )
        md.to_mmd(
            overrides={
    ...
    def test_collection_not_set(self):
        """ToDo: Add docstring"""
        md = Nc_to_mmd(
            os.path.abspath("tests/data/reference_nc_missing_collection.nc"),
            check_only=True,
        )
    ...
    def test_title_one_language_only(self):
        ...
        md = Nc_to_mmd(
            os.path.abspath("tests/data/reference_nc_id_missing.nc"), check_only=True
        )
        ncin = Dataset(md.netcdf_file)
        value = md.get_titles(mmd_yaml["title"], ncin)
        self.assertEqual(type(value), list)
        ...
        self.assertTrue("title" in value[0].keys())
        self.assertEqual(
            md.missing_attributes["errors"][0], "title_no is a required attribute"
        )
    ...
    def test_alternate_identifier_wrong_format(self):
        ...
        md = Nc_to_mmd(
            os.path.abspath("tests/data/reference_nc_with_altID.nc"), check_only=True
        )
        ncin = Dataset(md.netcdf_file, "w", diskless=True)
    ...
    def test_alternate_identifier_multiple(self):
        ...
        md = Nc_to_mmd(
            os.path.abspath("tests/data/reference_nc_with_altID_multiple.nc"),
            check_only=True,
        )
        ncin = Dataset(md.netcdf_file)
        value = md.get_alternate_identifier(mmd_yaml["alternate_identifier"], ncin)
    ...
    def test_use_defaults_for_personnel(self):
        ...
        md = Nc_to_mmd(
            os.path.abspath(
                os.path.abspath("tests/data/reference_nc_missing_attrs.nc")
            ),
            check_only=True,
        )
        ncin = Dataset(md.netcdf_file)
    ...
    def test_missing_temporal_extent_but_start_provided_in_dict(self):
        ...
        self.assertEqual(
            md.metadata["temporal_extent"]["start_date"], "1850-01-01T00:00:00Z"
        )
    ...
    def test_missing_temporal_extent_but_start_and_end_provided_in_dict(self):
        ...
        self.assertEqual(
            md.metadata["temporal_extent"]["end_date"], "1950-01-01T00:00:00Z"
        )
    ...
    def test_temporal_extent_two_startdates(self):
        ...
        md = Nc_to_mmd(
            os.path.abspath("tests/data/reference_nc_attrs_multiple.nc"),
            check_only=True,
        )
        ncin = Dataset(md.netcdf_file)
        value = md.get_temporal_extents(mmd_yaml["temporal_extent"], ncin)
    ...
    def test_temporal_extent_two_startdates_one_wrong(self):
        ...
        self.assertIn(
            template.format(invalid_start), md.missing_attributes["errors"][0]
        )
    ...
    def test_get_activity_type_invalid(self):
        ...
        assert (
            "The ACDD attribute 'source' in MMD attribute 'activity_type'"
            in md.missing_attributes["errors"][0]
        )
    ...
    def test_platform_resource_not_MMD(self):
        ...
        md = Nc_to_mmd(
            os.path.abspath("tests/data/reference_nc_missing_keywords_vocab.nc"),
            check_only=True,
        )
    ...
    def test_missing_vocabulary_platform_instrument_short_name(self):
        ...
        value = md.get_platforms(mmd_yaml["platform"], ncin)
        self.assertEqual(
            value[0]["resource"], "https://www.wmo-sat.info/oscar/satellites/view/342"
        )
    ...
    def test_wrong_platform_name(self):
        ...
        ncin = Dataset(md.netcdf_file, "w", diskless=True)
        ncin.platform = (
            "Suomi National Polar-orbiting Partnership (Suomi NPP)(Too Many)(SNPP)"
        )
        value = md.get_platforms(mmd_yaml["platform"], ncin)
        self.assertEqual(value, [])
        self.assertEqual(
    ...
    def test_instrument_name_extra_parentheses(self):
        ...
        self.assertEqual(
            value[0]["instrument"]["long_name"], "Synthetic Aperture Radar (C-band)"
        )
    ...
    def test_keywords_missing(self):
        ...
        md = Nc_to_mmd(
            os.path.abspath("tests/data/reference_nc_fail.nc"), check_only=True
        )
        ncin = Dataset(md.netcdf_file)
        ...
        self.assertEqual(
            md.missing_attributes["errors"][1], "keywords is a required ACDD attribute"
        )
    ...
    def test_spaces_around_keywords_are_stripped(self):
        ...
        self.assertEqual(
            data[0]["keyword"][0], "Earth Science > Atmosphere > Atmospheric radiation"
        )
    ...
    def test_keywords_standard_name_not_in_CFSTDN(self):
        ...
        md = Nc_to_mmd(
            os.path.abspath("tests/data/reference_nc_non_cf_standard_name.nc"),
            check_only=True,
        )
        ncin = Dataset(md.netcdf_file)
        md.get_keywords(mmd_yaml["keywords"], ncin)
    ...
    def test_keywords(self):
        ...
        self.assertEqual(
            value[3]["resource"], "https://vocab.met.no/mmd/Keywords_Vocabulary/CFSTDN"
        )
    ...
    def test_platforms(self):
        ...
        value = md.get_platforms(mmd_yaml["platform"], ncin)
        self.assertEqual(
            value[0]["long_name"], "Suomi National Polar-orbiting Partnership"
        )
        self.assertEqual(
    ...
    def test_platform_with_gcmd_vocabulary(self):
        ...
        md = Nc_to_mmd(
            os.path.abspath("tests/data/reference_nc_gcmd_platform.nc"), check_only=True
        )
        ncin = Dataset(md.netcdf_file)
        value = md.get_platforms(mmd_yaml["platform"], ncin)
        ...
        self.assertEqual(
            value[0]["instrument"]["long_name"], "Synthetic Aperture Radar"
        )
    ...
    def test_projects_with_short_name(self):
        ...
        md = Nc_to_mmd(
            os.path.abspath("tests/data/reference_nc_project_with_short_name.nc"),
            check_only=True,
        )
        ncin = Dataset(md.netcdf_file)
        value = md.get_projects(mmd_yaml["project"], ncin)
    ...
    def test_projects_missing(self):
        ...
        md = Nc_to_mmd(
            os.path.abspath("tests/data/reference_nc_missing_project.nc"),
            check_only=True,
        )
        ncin = Dataset(md.netcdf_file)
        value = md.get_projects(mmd_yaml["project"], ncin)
    ...
    def test_projects_malformed(self):
        ...
        md = Nc_to_mmd(
            os.path.abspath("tests/data/reference_nc_malformed_project.nc"),
            check_only=True,
        )
        ncin = Dataset(md.netcdf_file)
    ...
    def test_dataset_citation_missing_attrs(self):
        ...
        self.assertEqual(
            value[0]["author"], "DIVISION FOR OBSERVATION QUALITY AND DATA PROCESSING"
        )
    ...
    def test_dataset_citation_as_kwarg(self):
        ...
        value = md.get_dataset_citations(
            mmd_yaml["dataset_citation"], ncin, dataset_citation=dc
        )
    ...
    def test_dataset_citation(self):
        ...
        self.assertEqual(value[0]["publication_date"], "2020-11-27T14:05:56Z")
        self.assertEqual(
            value[0]["title"],
            "Direct Broadcast data processed in satellite swath to L1C.",
        )
        self.assertEqual(value[0]["publisher"], "Norwegian Meteorological Institute")
        self.assertEqual(
            value[0]["url"],
    ...
    def test_get_metadata_identifier(self):
        ...
        mmd_yaml["metadata_identifier"]["acdd"] = {
            "jkhakjh": {},
            "naming_authority": {},
        }
        with self.assertRaises(AttributeError) as e:
    ...
    def test_to_mmd_warning_not_empty(self):
        ...
        mmd_yaml["dummy_field"]["acdd_ext"] = {
            "dummy_field": {"comment": "no comment", "default": "hei"}
        }
        md = Nc_to_mmd(os.path.abspath("tests/data/reference_nc.nc"), check_only=True)
    ...
    def test_create_requires_naming_authority(self):
        ...
        """
        self.assertTrue(
            "naming_authority is a required attribute" in str(context.exception)
        )
    ...
    def test__to_mmd__invalid_uuid(self):
        ...
        # The id attribute is not a uuid
        md = Nc_to_mmd(
            os.path.abspath("tests/data/reference_nc_id_not_uuid.nc"), check_only=True
        )
        with self.assertRaises(AttributeError):
            md.to_mmd()
        self.assertFalse(Nc_to_mmd.is_valid_uuid(md.metadata["metadata_identifier"]))
    ...
    def test__to_mmd__error_if_acdd_id_is_invalid(self):
        ...
        self.assertEqual(
            md.missing_attributes["errors"][1], ("id ACDD attribute is not valid.")
        )
        self.assertFalse(Nc_to_mmd.is_valid_uuid(md.metadata["metadata_identifier"]))
        ncin = Dataset(md.netcdf_file)
    ...
    def test_all_valid_nc_files_passing(self):
        ...
        valid_files = [
            ...
            os.path.join(
                pathlib.Path.cwd(), "tests/data/reference_nc_attrs_multiple.nc"
            ),
        ]
    ...
    def test_ACDD_attr__date_created(self):
        ...
        self.assertIn(
            {"datetime": ncin.date_created, "type": "Created"}, data["update"]
        )
    ...
    def test_get_metadata_updates_wrong_input_dict(self):
        ...
        self.assertTrue(
            "ACDD attribute inconsistency in mmd_elements.yaml"
    ...
    def test_spatial_repr_fail(self):
        ...
        tempfile.mkstemp()[1]
        md = Nc_to_mmd(
            os.path.abspath("tests/data/reference_nc_missing_spatial_repr.nc"),
            check_only=True,
        )
        with self.assertRaises(AttributeError):
            md.to_mmd()
    ...
    def test_check_attributes_not_empty(self):
        ...
        self.assertIn(
            "Global attribute geospatial_bounds is empty - please correct.",
            str(e.exception),
        )
    ...
    def test_institution_name_parsing(self):
        ...
        self.assertEqual(
            data[0]["data_center_name"]["long_name"],
    ...
    def test_acdd_references_as_related_information1(self):
        ...
        self.assertEqual(
            data[1]["resource"], "https://ieeexplore.ieee.org/document/7914752"
        )
    ...
    def test_acdd_references_invalid_url(self):
        ...
        self.assertEqual(
            md.missing_attributes["errors"][0], "references must contain valid uris"
        )
        self.assertEqual(
            md.missing_attributes["errors"][1], "references must contain valid uris"
        )
