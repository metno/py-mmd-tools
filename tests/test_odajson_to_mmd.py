...
class TestODA2MMD(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.reference_in_json = str(
            current_dir / 'tests' / 'data' / 'reference_json.json'
        )
        self.reference_out_xml = str(
            current_dir / 'tests' / 'data' / 'reference_mmd_from_json.xml'
        )
        self.xml_template = str(
            current_dir / 'templates' / 'oda_to_mmd_template.xml'
        )
        ...
        self.json_with_invalid_elements = str(
            current_dir / 'tests' / 'data' / 'json_invalid_element.json'
        )
    ...
    def test_lowercase(self):
        ...
        self.assertEqual(
            odajson_to_mmd._lowercase(dictin),
            {'toto': 'TOTO', 'tata': 2, 'tutu': 'Hello'}
        )
        dictin = [{'Toto': 'TOTO', 'tata': 2, 'TUTU': 'Hello'}]
    ...
    def test_to_mmd_1(self):
        ...
        odajson_to_mmd.to_mmd(
            input_data=self.reference_in_json,
            output_file=tested,
    ...
    def test_to_mmd_3(self):
        ...
        tested = tempfile.mkstemp()[1]
        self.assertRaises(
            TypeError, odajson_to_mmd.to_mmd, self.not_a_file, tested, self.xml_template
        )
    ...
    def test_to_mmd_4(self):
        ...
        tested = tempfile.mkstemp()[1]
        self.assertRaises(
            jinja2.exceptions.TemplateNotFound, odajson_to_mmd.to_mmd,
            self.reference_in_json, tested, self.not_a_file
        )
    ...
    def test_to_mmd_5(self):
        ...
        tested = tempfile.mkstemp()[1]
        self.assertRaises(
            TypeError, odajson_to_mmd.to_mmd, self.reference_in_json,
            tested, self.xml_template, xsd_validation=True
        )
    ...
    def test_to_mmd_6(self):
        ...
        tested = tempfile.mkstemp()[1]
        self.assertRaises(
            FileNotFoundError, odajson_to_mmd.to_mmd, self.reference_in_json,
            tested, self.xml_template, xsd_validation=True, xsd_schema=self.not_a_file
        )
    ...
    def test_to_mmd_7(self):
        ...
        self.assertIs(
            odajson_to_mmd.to_mmd(
                self.json_with_invalid_elements, tested, self.xml_template,
                xsd_validation=True, xsd_schema=self.reference_xsd
            ), False
        )
    ...
    def test_prepare_elements_3(self):
        ...
        self.assertEqual(
            out['geographic_extent']['latitude_max'],
            default['geographic_extent']['latitude_max']
        )
        self.assertEqual(
            out['geographic_extent']['latitude_min'],
            default['geographic_extent']['latitude_min']
        )
        self.assertEqual(
            out['geographic_extent']['longitude_max'],
            default['geographic_extent']['longitude_max']
        )
        self.assertEqual(
            out['geographic_extent']['longitude_min'],
    ...
    def test_process_station_1(self, mock_get, mock_response, mock_to_mmd):
        ...
        odajson_to_mmd.process_station(
            '18269', 'HAUGENSTUA', 'outdir', self.default,
        ...
        self.assertRaises(
            FileNotFoundError, odajson_to_mmd.process_station, '18269', 'HAUGENSTUA',
    ...
    def test_retrieve_frost_stations__invalid_url(self):
        ...
        self.assertRaises(
            HTTPError, odajson_to_mmd.retrieve_frost_stations,
