...
class Nc_to_mmd(object):
    ...
    def get_metadata_updates(self, mmd_element, ncin):
        ...
        if DATE_CREATED not in ncin.ncattrs():
            self.missing_attributes["errors"].append("ACDD attribute %s is required" % DATE_CREATED)
            return
    ...
    def get_CFSTDN_keywords(self, ncin):
        ...
        for key in ncin.variables.keys():
            if "standard_name" in ncin.variables[key].ncattrs():
                if (
                    ncin.variables[key].standard_name not in ["longitude", "latitude", "time"]
                    and ncin.variables[key].standard_name not in varlist
                ):
    ...
    def get_metadata_identifier(self, mmd_element, ncin, **kwargs):
        ...
        if (
            len(acdd_key) != 2
            or self.ACDD_ID not in acdd_key
            or self.ACDD_NAMING_AUTH not in acdd_key
        ):
            raise AttributeError(
    ...
    def to_mmd(
        ...
        **kwargs,
        ...
        # Set Activity_Type
        self.metadata["activity_type"] = self.get_activity_type(mmd_yaml.pop("activity_type"), ncin)
